...
const Data = [
  // Sample data
  { id: 1, invoiceNum: "INV102397", name: "Jack", date: "Oct 17, 2024", dueDate: "Nov 17, 2024", amount: "250$", status: "paid", payment: "card", image: pic },
  { id: 2, invoiceNum: "INV102398", name: "Jane", date: "Oct 18, 2024", dueDate: "Nov 18, 2024", amount: "300$", status: "pending", payment: "cash", image: pic },
  { id: 3, invoiceNum: "INV102399", name: "Bob", date: "Oct 19, 2024", dueDate: "Nov 19, 2024", amount: "150$", status: "paid", payment: "card", image: pic },
  { id: 4, invoiceNum: "INV102400", name: "Alice", date: "Oct 20, 2024", dueDate: "Nov 20, 2024", amount: "400$", status: "pending", payment: "paypal", image: pic },
  { id: 5, invoiceNum: "INV102401", name: "Charlie", date: "Oct 21, 2024", dueDate: "Nov 21, 2024", amount: "500$", status: "paid", payment: "check", image: pic },
];

const BillingManagement = () => {
  const router = useRouter();
  const nextpg = () => {
    router.push("/billingManagment2");
  };
  
  const [modalOpen, setModalOpen] = useState(false);
  const [date, setDate] = useState(null);
  
  const openModal = (dueDate) => {
    setDate(dueDate);
    setModalOpen(true);
  };
        ...
        <div className='bg-white rounded-md p-3 shadow-xl mt-10'>
          <div className='flex justify-between items-center'>
            <div>
              ...
              <p className='text-xs text-gray-500 pt-2'>Accounts with +99 invoices are not accepted.</p>
            </div>
          ...
          <div className='mt-5 flex justify-between items-center'>
            <div>
              <button className='border p-2 rounded-l-md text-xs text-gray-500'>24 hours</button>
              <button className='border p-2 text-xs text-gray-500'>7 days</button>
              <button className='border p-2 text-xs text-gray-500'>30 days</button>
              <button className='border rounded-r-md p-2 text-xs text-gray-500'>customize</button>
            </div>
            ...
            <tbody>
              {Data.map((item) => (
                <tr key={item.id}>
                  ...
                  <td className="border-b px-4 py-2 text-center">{item.invoiceNum}</td>
                  <td className="border-b px-4 py-2 text-center flex items-center space-x-2">
                    <Image src={item.image} alt={item.name} className='rounded-full w-7 h-7' />
                    <span>{item.name}</span>
                  </td>
                  <td className="border-b px-4 py-2 text-center">{item.date}</td>
                  <td className="border-b px-4 py-2 text-center cursor-pointer" onClick={() => openModal(item.dueDate)}>{item.dueDate}</td>
                  <td className="border-b px-4 py-2 text-center">{item.amount}</td>
                  <td className="border-b px-4 py-2 text-center">{item.status}</td>
                  <td className="border-b px-4 py-2 text-center">{item.payment}</td>
          ...
          <div className="flex justify-between items-center mt-6">
            ...
            <button className="flex items-center p-2 border rounded-md text-sm text-gray-500 hover:bg-gray-200" onClick={nextpg}>
              Next
          ...
          date && (
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DemoContainer components={['DatePicker']}>
                <DatePicker label={`Due Date: ${date}`} />
              </DemoContainer>
            </LocalizationProvider>
            <button className='bg-blue-400 text-white rounded-md w-full mt-3 p-2'>Download</button>
            </div>
          )
        }
      </Modal>
